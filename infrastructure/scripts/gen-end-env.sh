...
if [ "$ENV" = "prod" ]; then
    ...
    set_env $BACKEND_ENV "SEAWEEDFS_MASTER" "http://seaweedfs-master:$INTERNAL_SEAWEEDFS_MASTER"
    set_env $BACKEND_ENV "SEAWEEDFS_VOLUME" "http://seaweedfs-volume:$INTERNAL_SEAWEEDFS_VOLUME"
    set_env $BACKEND_ENV "OPENSEARCH_URL" "http://opensearch-node:$INTERNAL_OPENSEARCH_NODE_P1"
