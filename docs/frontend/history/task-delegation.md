...
Reacts to [Task Delegation](../../backend/history/task-delegation-plan.md) and
[the `tasks` + `task_history` rewrite](../../backend/history/task-history-rewrite.md), both
...
- **`src/api/` has only `admin.ts` and `search.ts`.** There is still **no
  `users.ts`** — the directory catch-up task (done 2026-08-17, file removed)
...
in the directory catch-up task (done 2026-08-17, file removed).
...
- `docs/backend/reference.md` — "Tasks (delegation)" and "Users (directory)"
