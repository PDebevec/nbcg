...
### B1 — v2 skeleton: types, labels, vocabularies, route (M)
...
### B2 — rule evaluator + conformance fixture (S)
...
|---|---|---|---|
| `summaryNote` accepted | add the Summary field back to the editor and the record page (removed in `cd8e5bd`) | — | — |
...
| New fields | rendered by the schema-driven form | must support `quantity` | optional explicit mapping + reindex |
