export default {
  nav: {
    home: 'Početna',
    about: 'O nama',
    terms: 'Uslovi korišćenja',
    advancedSearch: 'Napredna pretraga',
    contact: 'Kontakt',
    profile: 'Profil',
  },

  common: {
    library: 'Digitalna biblioteka',
    readMore: 'Saznaj više',
    search: 'Pretraga',
    reset: 'Poništi',
    back: 'Nazad',
    backHome: 'Nazad na početnu',
  },

  index: {
    searchPlaceholder: 'Pretraži po naslovu, autoru, temi …',
    fullTextOn: 'Pretraga cijelog teksta uključena',
    fullTextOff: 'Pretraga cijelog teksta isključena',
    searchTypes: {
      all: 'Sve',
      books: 'Knjige',
      periodicals: 'Periodika',
      manuscripts: 'Rukopisi',
    },
    collectionsTitle: 'Pretraga po kolekcijama',
    collections: {
      books: 'Knjige',
      newspapers: 'Novine',
      magazines: 'Časopisi',
      manuscripts: 'Rukopisi i dokumenta',
      maps: 'Karte i planovi',
      posters: 'Plakati i likovna građa',
      photographs: 'Fotografije i razglednice',
      audiovisual: 'Muzička i video građa',
    },
    thematicTitle: 'Pretraga po temama',
    newestTitle: 'Nedavno dodato',
    thematic: {
      oldRareBooks: {
        title: 'Stare i rijetke knjige',
        description: 'Izbor najvrednijih i najstarijih naslova iz fonda Nacionalne biblioteke.',
      },
      montenegrinPress: {
        title: 'Crnogorska periodika',
        description: 'Istorijske novine i časopisi štampani na teritoriji Crne Gore.',
      },
      cetinjeHeritage: {
        title: 'Cetinjska baština',
        description: 'Građa vezana za istoriju i kulturu prijestonice Cetinje.',
      },
      cartography: {
        title: 'Kartografska baština',
        description: 'Istorijske karte i planovi crnogorskih gradova i teritorija.',
      },
      artAndPosters: {
        title: 'Umjetnost i plakati',
        description: 'Likovna građa, plakati i promotivni materijali iz fonda biblioteke.',
      },
      folkHeritage: {
        title: 'Narodno stvaralaštvo',
        description: 'Zapisi i publikacije o crnogorskoj tradiciji i narodnom stvaralaštvu.',
      },
      njegos: {
        title: 'Njegoš i njegovo doba',
        description: 'Djela Petra II Petrovića Njegoša i građa o njegovom životu i epohi.',
      },
    },
    aboutKicker: 'O nama',
    aboutTitle: 'O Digitalnoj biblioteci Crne Gore',
    aboutP1:
      'Digitalna biblioteka Crne Gore je projekat Nacionalne biblioteke Crne Gore „Đurđe Crnojević“ zahvaljujući kojoj su digitalizovani i prezentovani najvredniji primjerci bibliotečke građe iz kolekcija Nacionalne biblioteke u kojima se čuvaju stare i rijetke knjige, rukopisi, dokumenta, novine, časopisi, karte, plakati i likovna, muzička i video građa…',
    aboutP2:
      'Cilj ovog projekta je zaštita originalnih dokumenata i njihovo predstavljanje u digitalnom obliku, odnosno obezbjeđivanje njihove dostupnosti najširem krugu korisnika u čitavom svijetu putem interneta.',
    aboutP3:
      'Najveći dio građe koji se nalazi na portalu Digitalne biblioteke digitalizovan je u Centru za mikrofilmovanje i digitalizaciju Nacionalne biblioteke, koji je formiran 2008. godine u okviru Odjeljenja za razvoj bibliotečke djelatnosti. U ovom centru mikrofilmovani su neki od najvažnijih naslova crnogorske periodike, kao što su Glas Crnogorca, Crnogorac, Crnogorka, Grlica, ali i skenirano više od 400 naslova periodike, stare i rijetke knjige, rukopisa, fotografija i plakata.',
    aboutP4:
      'Projekat Digitalna biblioteka Crne Gore realizovan je uz pomoć Ministarstva kulture Crne Gore, a u skladu sa Nacionalnim programom za digitalizaciju, koji je Vlada Crne Gore usvojila 2008. godine. Svrha Nacionalnog programa za digitalizaciju je da omogući ujednačeni pristup digitalizaciji bibliotečke građe u Crnoj Gori. Program je usmjeren ka dugoročnoj strategiji digitalizacije, a cilj mu je da podstakne unaprjeđenje institucionalne, tehnološke, stručne i organizacione infrastrukture za projekte digitalizacije.',
  },

  about: {
    kicker: 'O nama',
    title: 'O Digitalnoj biblioteci Crne Gore',
  },

  terms: {
    title: 'Uslovi korišćenja',
    p1: 'Nacionalna biblioteka Crne Gore „Đurđe Crnojević“ zadržava sva autorska prava na digitalizovani i objavljeni materijal na portalu Digitalna biblioteka Crne Gore.',
    p2: 'Korisnici portala Digitalna biblioteka Crne Gore mogu besplatno koristiti digitalizovani materijal pod uslovom da se na taj način ne ostvaruje materijalna korist. Materijal se ne može distribuirati, razmjenjivati ili prodavati bez pismene saglasnosti vlasnika autorskih prava.',
  },

  advanced: {
    title: 'Napredna pretraga',
    titleField: 'Naslov',
    author: 'Autor',
    publisher: 'Izdavač',
    materialType: 'Vrsta građe',
    language: 'Jezik',
    yearFrom: 'Godina od',
    yearTo: 'Godina do',
    types: {
      all: 'Sve vrste',
    },
    languages: {
      all: 'Svi jezici',
    },
  },

  contact: {
    title: 'Kontakt',
    phone: 'Telefon',
    email: 'E-mail',
    address: 'Adresa',
    addressValue: 'Nacionalna biblioteka Crne Gore „Đurđe Crnojević“, Cetinje, Crna Gora',
  },

  auth: {
    login: 'Prijava',
    logout: 'Odjava',
    sessionExpired: 'Vaša sesija je istekla. Nesačuvane izmjene ostaju na ovoj stranici — prijavite se ponovo da nastavite.',
  },

  profile: {
    title: 'Korisnički profil',
    unavailableTitle: 'Prijava nije dostupna',
    unavailableText:
      'Korisnički nalozi će uskoro biti omogućeni na portalu Digitalne biblioteke Crne Gore.',
    username: 'Korisničko ime',
    email: 'E-pošta',
    fullName: 'Ime',
    roles: 'Uloge',
    noRoles: 'Nema dodijeljenih uloga',
  },

  footer: {
    mission: 'Naša misija',
    missionText:
      'Cilj ovog projekta je zaštita originalnih dokumenata i njihovo predstavljanje u digitalnom obliku, odnosno obezbjeđivanje njihove dostupnosti najširem krugu korisnika u čitavom svijetu putem interneta.',
    navigation: 'Navigacija',
    contact: 'Kontakt',
    phone: 'Tel: + 382 41 234 243, lokal 13',
    email: "e-mail: info{'@'}dlib.me",
    copyright: 'Digitalna biblioteka Crne Gore.',
  },

  catalog: {
    kicker: 'Digitalna biblioteka',
    title: 'Katalog građe',
    searchWithin: 'Pretraži unutar kataloga …',
    fullTextOn: 'Pretraga cijelog teksta uključena',
    fullTextOff: 'Pretraga cijelog teksta isključena',
    clearSearch: 'Očisti pretragu',
    hideSearch: 'Sakrij pretragu',
    showSearch: 'Prikaži pretragu',
    filters: 'Filteri',
    itemType: 'Vrsta građe',
    language: 'Jezik',
    allTypes: 'Sve vrste',
    allLanguages: 'Svi jezici',
    period: 'Period',
    resetFilters: 'Poništi filtere',
    showing: 'Prikazano',
    of: 'od',
    items: 'jedinica',
    sortBy: 'Sortiraj po',
    view: 'Pregled',
    details: 'Detalji',
    eras: {
      all: 'Sve',
      pre1800: 'Prije 1800',
      c19: '1800–1900',
      e1900: '1900–1950',
      e1950: '1950–2000',
      post2000: 'Poslije 2000',
    },
    sort: {
      relevance: 'Relevantnost',
      newest: 'Najnovije',
    },
  },

  record: {
    backToCatalog: 'Nazad na katalog',
    notFound: 'Jedinica nije pronađena.',
    bibliographic: 'Bibliografski podaci',
    authors: 'Autori',
    notes: 'Napomene',
    series: 'Edicija',
    classification: 'Klasifikacija',
    attachments: 'Prilozi',
    links: 'Na internetu',
    download: 'Preuzmi',
    share: 'Podijeli',
    openInNewTab: 'Otvori u novoj kartici',
    linkCopied: 'Link je kopiran',
    noPreview: 'Pregled nije dostupan za ovaj fajl.',
    noFiles: 'Ovaj zapis nema priloženih fajlova.',
    zoomIn: 'Uvećaj',
    zoomOut: 'Umanji',
    rotate: 'Rotiraj',
    fullscreen: 'Cijeli ekran',
    exitFullscreen: 'Izađi iz cijelog ekrana',
    tabs: {
      main: 'Glavni metapodaci',
      all: 'Svi metapodaci',
    },
    relatedCollections: 'Povezane zbirke',
    noCollections: 'Ovaj zapis još nije dio nijedne zbirke.',
    fields: {
      title: 'Naslov',
      mediumDesignation: 'Opšta oznaka građe',
      subtitle: 'Podnaslov',
      parallelTitle: 'Paralelni naslov',
      titleInOtherScript: 'Naslov na drugom pismu',
      titleByAnotherAuthor: 'Naslov drugog autora',
      responsibility: 'Odgovornost',
      addResponsibility: 'Dodatna odgovornost',
      edition: 'Izdanje',
      publisher: 'Izdavač',
      place: 'Mjesto',
      year: 'Godina',
      publicationDate2: 'Drugi datum',
      placeOfManufacture: 'Mjesto štampanja',
      manufacturer: 'Štamparija',
      extent: 'Obim',
      otherPhysicalDetails: 'Ostali fizički detalji',
      dimensions: 'Dimenzije',
      cartographic: 'Matematički podaci',
      numbering: 'Numeracija',
      musicEdition: 'Muzičko izdanje',
      isbn: 'ISBN',
      issn: 'ISSN',
      ismn: 'ISMN',
      cobissId: 'COBISS ID',
      language: 'Jezik',
      originalLanguage: 'Izvorni jezik',
      translationLanguages: 'Jezik sažetka',
      country: 'Država',
      materialType: 'Vrsta građe',
      recordType: 'Vrsta zapisa',
      bibLevel: 'Bibliografski nivo',
      docTypology: 'Tipologija dokumenta',
      illustrations: 'Ilustracije',
      contentTypes: 'Vrsta sadržaja',
      literaryForm: 'Književna forma',
      biography: 'Biografija',
      seriesTitle: 'Naslov edicije',
      seriesSubtitle: 'Podnaslov edicije',
      seriesResponsibility: 'Odgovornost za ediciju',
      seriesIssn: 'ISSN edicije',
      volume: 'Kolo / svezak',
    },
  },

  error: {
    notFoundTitle: 'Ups. Ovdje nema ničega…',
    goHome: 'Nazad na početnu',
  },

  forbidden: {
    title: 'Pristup odbijen',
    text: 'Nemate dozvolu za pregled ove stranice.',
  },

  admin: {
    title: 'Administracija',
    backToSite: 'Nazad na portal',
    nav: {
      dashboard: 'Pregled',
      drafts: 'Nacrti',
      records: 'Zapisi',
      import: 'Uvoz',
      stats: 'Statistika',
      tasks: 'Zadaci',
    },
    visibility: {
      PUBLIC: 'Javno',
      PRIVATE: 'Privatno',
      HIDDEN: 'Skriveno',
    },
    dashboard: {
      title: 'Pregled',
      statsFailed: 'Učitavanje statistike nije uspjelo.',
      quickActions: 'Brze akcije',
      newDraft: 'Novi nacrt',
      newRecord: 'Novi zapis',
      runImport: 'Pokreni uvoz',
      myTasks: 'Čeka na mene',
      myTasksCount: 'Otvorenih zadataka dodijeljenih vama: {count}',
      myTasksNone: 'Ništa ne čeka na vas.',
      myTasksFailed: 'Učitavanje vaših zadataka nije uspjelo.',
      openInbox: 'Otvori zadatke',
    },
    items: {
      searchPlaceholder: 'Pretraga po naslovu, autoru, COBISS ID …',
      selected: 'Izabrano: {count}',
      setVisibility: 'Postavi vidljivost',
      publish: 'Objavi',
      toDraft: 'Vrati u nacrt',
      delete: 'Obriši',
      edit: 'Uredi',
      openPublic: 'Otvori javnu stranicu',
      empty: 'Nema pronađenih zapisa.',
      loadFailed: 'Učitavanje zapisa nije uspjelo.',
      actionFailed: 'Akcija nije uspjela.',
      confirmTitle: 'Potvrda',
      cancel: 'Otkaži',
      confirm: 'Potvrdi',
      deleteConfirm: 'Obrisati {count} zapis(a)? Priložene datoteke će takođe biti obrisane. Ovo se ne može poništiti.',
      deleted: 'Obrisano: {count} zapis(a).',
      transitioned: 'Zapisi su premješteni.',
      visibilityUpdated: 'Vidljivost ažurirana za {count} zapis(a).',
      columns: {
        title: 'Naslov',
        author: 'Autor',
        year: 'Godina',
        visibility: 'Vidljivost',
        updated: 'Izmijenjeno',
        createdBy: 'Kreirao/la',
      },
      creatorFilter: 'Kreator',
      creatorFilterEmpty: 'Nema pronađenih korisnika.',
      openTask: 'Ima otvoren zadatak',
    },
    users: {
      title: 'Imenik korisnika',
      refresh: 'Osvježi imenik korisnika',
      profileCount: 'Korisnika u imeniku: {count}',
      lastSync: 'Posljednja sinhronizacija: {when}',
      neverSynced: 'Sinhronizacija još nije pokrenuta.',
      syncRunning: 'Sinhronizacija u toku …',
      lastError: 'Greška pri posljednjoj sinhronizaciji ({when}): {message}',
      syncQueued: 'Osvježavanje imenika korisnika je pokrenuto.',
      syncFailed: 'Pokretanje osvježavanja imenika korisnika nije uspjelo.',
      statusFailed: 'Učitavanje statusa sinhronizacije nije uspjelo.',
    },
    extraction: {
      EXTRACTED: 'Tekst iz PDF-a izdvojen',
      NOT_EXTRACTED: 'Izdvajanje teksta iz PDF-a nije obavljeno ili je u toku',
      NO_TEXT: 'PDF ne sadrži tekst (samo slike?) — razmislite o ponovnom učitavanju sa OCR-om',
      GARBAGE: 'Izdvojeni tekst iz PDF-a je nečitljiv — razmislite o ponovnom učitavanju sa OCR-om',
    },
    stats: {
      title: 'Statistika',
      loadFailed: 'Učitavanje statistike nije uspjelo.',
      invalidRange: 'Početni datum ne smije biti poslije krajnjeg.',
      rangeTooWide: 'Raspon ne smije biti duži od godinu dana.',
      period: 'Period',
      presets: {
        last7: 'Zadnjih 7 dana',
        last30: 'Zadnjih 30 dana',
        last90: 'Zadnjih 90 dana',
        lastYear: 'Zadnja godina',
      },
      activity: 'Aktivnost katalogizacije',
      usage: 'Pregledi i preuzimanja',
      byUser: 'Po korisniku',
      topItems: 'Najgledanije i najpreuzimanije',
      mostViewed: 'Najgledaniji zapisi',
      mostDownloaded: 'Najpreuzimaniji zapisi',
      topFiles: 'Najpreuzimanije datoteke',
      noData: 'Nema podataka za ovaj period.',
      usersEmpty: 'Nema aktivnosti u ovom periodu.',
      deletedItem: 'Obrisan zapis',
      deletedFile: 'Obrisana datoteka',
      tiles: {
        created: 'Kreirano',
        published: 'Objavljeno',
        updated: 'Uređeno',
        deleted: 'Obrisano',
        views: 'Pregledi',
        downloads: 'Preuzimanja',
      },
      columns: {
        user: 'Korisnik',
        created: 'Kreirano',
        published: 'Objavljeno',
        edited: 'Uređeno',
        deleted: 'Obrisano',
        total: 'Ukupno',
      },
    },
    history: {
      empty: 'Nema zabilježenih izmjena.',
      loadFailed: 'Učitavanje istorije izmjena nije uspjelo.',
      loadMore: 'Učitaj još',
      actions: {
        CREATE: 'Kreirano',
        UPDATE: 'Uređeno',
        PUBLISH: 'Objavljeno',
        UNPUBLISH: 'Vraćeno u nacrt',
        VISIBILITY_CHANGE: 'Promijenjena vidljivost',
        FILE_ADDED: 'Dodata datoteka',
        FILE_REMOVED: 'Uklonjena datoteka',
        RELATION_ADDED: 'Povezan srodni zapis',
        RELATION_REMOVED: 'Uklonjena veza sa srodnim zapisom',
        DELETE: 'Obrisano',
      },
      // Jedna oznaka po segmentu putanje metapodataka (v. DomainRecord u src/api/search.ts).
      fields: {
        title: 'Naslov',
        titleMediumDesignation: 'Opšta oznaka građe',
        titleByAnotherAuthor: 'Naslov drugog autora',
        subtitle: 'Podnaslov',
        parallelTitle: 'Paralelni naslov',
        titleInOtherScript: 'Naslov na drugom pismu',
        firstResponsibility: 'Autor / odgovornost',
        subsequentResponsibility: 'Sekundarna odgovornost',
        edition: 'Izdanje',
        cartographicMathematicalData: 'Matematički podaci',
        numberingAndDates: 'Numeracija i datumi',
        musicEditionStatement: 'Muzičko izdanje',
        cobissId: 'COBISS ID',
        collectionType: 'Vrsta zbirke',
        notes: 'Napomene',
        publication: 'Izdavanje',
        publisher: 'Izdavač',
        place: 'Mjesto',
        year: 'Godina',
        placeOfManufacture: 'Mjesto štampanja',
        manufacturerName: 'Štamparija',
        physicalDescription: 'Fizički opis',
        dimensions: 'Dimenzije',
        otherPhysicalDetails: 'Ostali fizički detalji',
        seriesTitle: 'Edicija',
        seriesSubtitle: 'Podnaslov edicije',
        seriesResponsibility: 'Odgovornost za ediciju',
        seriesIssn: 'ISSN edicije',
        seriesVolume: 'Sveska edicije',
        authors: 'Autor',
        corporateBodies: 'Korporativno tijelo',
        familyName: 'Prezime',
        firstName: 'Ime',
        prefix: 'Prefiks',
        romanNumerals: 'Rimski brojevi',
        dates: 'Godine',
        name: 'Naziv',
        role: 'Uloga',
        responsibility: 'Odgovornost',
        language: 'Jezik',
        originalLanguage: 'Izvorni jezik',
        translationLanguages: 'Jezik sažetka',
        country: 'Država',
        materialType: 'Vrsta građe',
        recordType: 'Vrsta zapisa',
        bibliographicLevel: 'Bibliografski nivo',
        documentTypology: 'Tipologija dokumenta',
        textualMaterialCodes: 'Kodirani podaci',
        illustrationCodes: 'Ilustracije',
        contentTypeCodes: 'Vrsta sadržaja',
        conferencePublication: 'Konferencijska publikacija',
        festschrift: 'Spomenica',
        indexIndicator: 'Ima registar',
        literaryForm: 'Književna forma',
        biographyCode: 'Biografija',
        isbn: 'ISBN',
        issn: 'ISSN',
        ismn: 'ISMN',
        publicationDate1: 'Datum izdavanja',
        publicationDate2: 'Drugi datum izdavanja',
        electronicLocation: 'Lokacija na internetu',
        url: 'URL',
        visibilityStatus: 'Vidljivost',
        itemType: 'Vrsta',
        file: 'Datoteka',
        child: 'Srodni zapis',
      },
    },
    edit: {
      title: 'Uređivanje zapisa',
      newTitle: 'Novi zapis',
      loadFailed: 'Zapis nije moguće učitati.',
      tabForm: 'Obrazac',
      tabJson: 'JSON',
      tabFiles: 'Datoteke',
      tabHistory: 'Istorija',
      tabTasks: 'Zadaci',
      assignTask: 'Dodijeli zadatak',
      sections: {
        identification: 'Identifikacija',
        responsibility: 'Odgovornost',
        publication: 'Izdavanje',
        physical: 'Fizički opis',
        series: 'Edicija',
        identifiers: 'Identifikatori',
        languages: 'Jezici i države',
        notes: 'Napomene i linkovi',
        advanced: 'Napredno',
        advancedCaption: 'Kodirani podaci (105), kartografski, serijski i muzički podaci',
      },
      fields: {
        title: 'Naslov',
        subtitle: 'Podnaslov',
        titleMediumDesignation: 'Opšta oznaka građe',
        titleByAnotherAuthor: 'Naslov drugog autora',
        parallelTitle: 'Paralelni naslovi',
        titleInOtherScript: 'Naslov na drugom pismu',
        materialType: 'Vrsta građe',
        recordType: 'Vrsta zapisa',
        bibliographicLevel: 'Bibliografski nivo',
        documentTypology: 'Tipologija dokumenta',
        firstResponsibility: 'Podatak o odgovornosti',
        subsequentResponsibility: 'Ostali podaci o odgovornosti',
        authors: 'Autori',
        corporateBodies: 'Korporativna tijela',
        publisher: 'Izdavač',
        place: 'Mjesto izdavanja',
        year: 'Godina',
        edition: 'Izdanje',
        publicationDate1: 'Datum 1',
        publicationDate2: 'Datum 2',
        placeOfManufacture: 'Mjesto štampanja',
        manufacturerName: 'Štamparija',
        physicalDescription: 'Obim',
        otherPhysicalDetails: 'Ostali fizički detalji',
        dimensions: 'Dimenzije',
        seriesTitle: 'Naslov edicije',
        seriesSubtitle: 'Podnaslov edicije',
        seriesResponsibility: 'Odgovornost za ediciju',
        seriesIssn: 'ISSN edicije',
        seriesVolume: 'Sveska u ediciji',
        isbn: 'ISBN',
        issn: 'ISSN',
        ismn: 'ISMN',
        language: 'Jezici',
        originalLanguage: 'Izvorni jezici',
        translationLanguages: 'Jezici sažetka',
        country: 'Države',
        notes: 'Napomene',
        electronicLocation: 'Lokacije na internetu',
        illustrationCodes: 'Ilustracije',
        contentTypeCodes: 'Vrste sadržaja',
        conferencePublication: 'Konferencijska publikacija',
        festschrift: 'Spomenica',
        indexIndicator: 'Ima registar',
        literaryForm: 'Književna forma',
        biographyCode: 'Biografija',
        cartographicMathematicalData: 'Matematički podaci (karte)',
        numberingAndDates: 'Numeracija i datumi (serijske publikacije)',
        musicEditionStatement: 'Podatak o muzičkom izdanju',
      },
      hints: {
        titleMediumDesignation: 'npr. „elektronski izvor“',
        firstResponsibility: 'Kako je odštampano na naslovnoj strani',
        year: 'Kako je odštampano (210d)',
        publicationDate1: 'Kodirana godina (100c)',
        publicationDate2: 'Završna godina / godina autorskog prava (100d)',
        physicalDescription: 'npr. „312 str.“',
        dimensions: 'npr. „24 cm“',
        originalLanguage: 'Za prevode',
        translationLanguages: 'Jezici sažetaka ili apstrakata',
        cobissId: 'COBISS ID',
        cobissIdNew: 'Neobavezno. Vezuje zapis za ovaj COBISS zapis.',
        cobissIdLocked: 'Ne može se mijenjati nakon kreiranja.',
        cartographicMathematicalData: 'Razmjera, projekcija …',
        numberingAndDates: 'npr. „God. 1 (1927)-“',
      },
      authors: {
        find: 'Pronađi postojećeg autora …',
        add: 'Dodaj autora',
        familyName: 'Prezime',
        firstName: 'Ime',
        prefix: 'Prefiks',
        romanNumerals: 'Brojevi',
        dates: 'Godine',
        datesPlaceholder: '1813-1851',
        role: 'Uloga',
        responsibility: 'Odgovornost',
      },
      corporate: {
        name: 'Naziv korporativnog tijela',
        add: 'Dodaj korporativno tijelo',
      },
      responsibility: {
        primary: 'Primarna',
        alternative: 'Alternativna',
        secondary: 'Sekundarna',
      },
      listHint: 'Unesite vrijednost i pritisnite Enter',
      addLine: 'Dodaj',
      addNote: 'Dodaj napomenu',
      addUrl: 'Dodaj link',
      remove: 'Ukloni',
      noMatch: 'Nema rezultata.',
      codeListsFailed:
        'Šifarnici nijesu učitani. Padajuće liste nude samo vrijednosti koje su već u upotrebi; tekstualna polja rade normalno.',
      titleRequired: 'Naslov je obavezan.',
      jsonHint: 'Kompletni metapodaci u JSON formatu. Nepoznata polja server odbacuje.',
      invalidJson: 'Neispravan JSON.',
      filesTitle: 'Priložene datoteke',
      chooseFiles: 'Izaberi datoteke',
      upload: 'Otpremi',
      uploaded: 'Datoteke su otpremljene.',
      noFiles: 'Nema priloženih datoteka.',
      deleteFileConfirm: 'Obrisati datoteku "{name}"?',
      fileDeleted: 'Datoteka je obrisana.',
      save: 'Sačuvaj',
      saved: 'Sačuvano.',
      conflictRefreshed:
        'Ovaj zapis je u međuvremenu izmijenio drugi korisnik, pa vaše izmjene nijesu sačuvane. Obrazac je osvježen najnovijim podacima — ponovo unesite izmjene ili izaberite „Ipak sačuvaj“ da prepišete tuđe izmjene.',
      saveAnyway: 'Ipak sačuvaj',
      dismiss: 'Zatvori',
    },
    tasks: {
      title: 'Zadaci',
      kinds: {
        REVIEW_PUBLISH: 'Pregled i objava',
        FIX_METADATA: 'Ispravka metapodataka',
        GENERAL: 'Opšte',
      },
      statuses: {
        OPEN: 'Otvoren',
        IN_PROGRESS: 'U toku',
        RETURNED: 'Vraćen',
        COMPLETED: 'Završen',
        CANCELLED: 'Otkazan',
      },
      actions: {
        CREATED: 'Kreiran',
        ASSIGNED: 'Preusmjeren',
        STATUS_CHANGED: 'Promijenjen status',
        RETURNED: 'Vraćen sa napomenama',
        COMMENTED: 'Komentar',
        UPDATED: 'Uređen',
        CLOSED_ON_PUBLISH: 'Automatski zatvoren — zapis objavljen',
      },
      fields: {
        kind: 'Vrsta',
        status: 'Status',
        assignedToUserId: 'Dodijeljen',
        title: 'Naslov',
        description: 'Opis',
        dueAt: 'Rok',
      },
      itemType: {
        DRAFT: 'Nacrt',
        RECORD: 'Zapis',
        gone: 'Zapis više ne postoji',
      },
      // Inbox
      scope: 'Prikaži',
      scopeMine: 'Dodijeljeno meni',
      scopeCreated: 'Kreirao/la sam',
      scopeAll: 'Svi zadaci',
      statusFilter: 'Status',
      hideClosed: 'Sakrij završene i otkazane',
      empty: 'Nema zadataka.',
      loadFailed: 'Učitavanje zadataka nije uspjelo.',
      truncated: 'Prikazano je prvih {limit} zadataka — suzite filtere da vidite ostale.',
      columns: {
        title: 'Zadatak',
        kind: 'Vrsta',
        status: 'Status',
        item: 'Zapis',
        assignedTo: 'Dodijeljen',
        createdBy: 'Kreirao/la',
        updated: 'Izmijenjeno',
      },
      openItem: 'Otvori zapis',
      openTask: 'Otvori zadatak',
      // Assignee picker
      picker: {
        label: 'Dodijeli korisniku',
        placeholder: 'Pretraga po imenu ili e-pošti …',
        empty: 'Nema odgovarajućih korisnika.',
        hintPublish: 'Samo korisnici koji mogu da objavljuju.',
        hintStaff: 'Bilo koji član osoblja koji može da uređuje.',
      },
      // Create dialog
      create: {
        title: 'Dodijeli zadatak',
        kind: 'Vrsta',
        taskTitle: 'Naslov',
        description: 'Opis (opciono)',
        submit: 'Dodijeli',
        titleRequired: 'Naslov je obavezan.',
        assigneeRequired: 'Izaberite kome se zadatak dodjeljuje.',
        created: 'Zadatak dodijeljen korisniku {name}.',
        failed: 'Kreiranje zadatka nije uspjelo.',
        defaultTitle: {
          REVIEW_PUBLISH: 'Spremno za pregled',
          FIX_METADATA: 'Potrebna ispravka metapodataka',
          GENERAL: '',
        },
      },
      // Detail page
      detail: {
        title: 'Zadatak',
        loadFailed: 'Zadatak nije moguće učitati.',
        assignedTo: 'Dodijeljen',
        createdBy: 'Kreirao/la',
        item: 'Zapis',
        created: 'Kreiran',
        completed: 'Završen',
        description: 'Opis',
        activity: 'Aktivnost',
        commentPlaceholder: 'Napišite komentar …',
        send: 'Pošalji',
        commentFailed: 'Slanje komentara nije uspjelo.',
        actionFailed: 'Akcija nije uspjela.',
        statusUpdated: 'Status je ažuriran.',
        start: 'Počni rad',
        complete: 'Označi kao završen',
        reopen: 'Ponovo otvori',
        cancel: 'Otkaži zadatak',
        cancelConfirm: 'Otkazati ovaj zadatak? Otkazan zadatak se ne može ponovo otvoriti.',
        cancelledHint: 'Ovaj zadatak je otkazan i više ne može mijenjati status. Otvorite novi zadatak.',
        publishHint: 'Objavljivanje zapisa automatski zatvara ovaj zadatak.',
        returnAction: 'Vrati sa napomenama',
        sendBack: 'Pošalji ponovo na pregled',
        reassign: 'Preusmjeri',
        returnDialog: {
          title: 'Vrati sa napomenama',
          hint: 'Zadatak se vraća izabranoj osobi zajedno sa vašim napomenama.',
          note: 'Napomene',
          noteRequired: 'Navedite šta treba promijeniti.',
          submit: 'Vrati',
          done: 'Zadatak vraćen korisniku {name}.',
        },
        reassignDialog: {
          title: 'Preusmjeri',
          submit: 'Preusmjeri',
          done: 'Zadatak preusmjeren korisniku {name}.',
        },
      },
      // History renderer
      history: {
        empty: 'Još nema aktivnosti.',
        loadFailed: 'Učitavanje istorije zadatka nije uspjelo.',
        loadMore: 'Učitaj još',
        toAssignee: 'korisniku {name}',
        unknownUser: 'Nepoznat korisnik',
      },
    },
    import: {
      title: 'COBISS uvoz',
      newImport: 'Novi uvoz',
      idsLabel: 'COBISS ID-jevi',
      idsHint: 'Jedan ili više ID-jeva, razdvojenih razmacima, zarezima ili novim redovima.',
      target: 'Uvezi kao',
      targetDraft: 'Nacrt',
      targetRecord: 'Zapis',
      start: 'Pokreni uvoz ({count})',
      started: 'Uvoz je pokrenut (posao #{jobId}).',
      jobs: 'Nedavni poslovi',
      noJobs: 'Nema nedavnih uvoza.',
      progress: '{processed}/{total} obrađeno · {succeeded} uspješno · {failed} neuspješno',
    },
  },
};
